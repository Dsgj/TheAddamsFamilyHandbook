...
| **Machine setup**     | Seven-step guide for presets, adjustments, high scores, H-4 ROM extras, utilities after a move and upgrades/upkeep, with suggested values, reasons, Handbook links and a field for what the machine is set to.                                  |
| **Care**              | Cleaning and upkeep by interval (weekly, monthly, twice a year, yearly) with the product or method per item, what the manual says, and a dated tick per item stored on the device.                                                              |
