...
**The Addams Family Handbook** is an interactive service manual for a single Bally *The Addams
Family* pinball machine. Type what the machine tells you, get the wiring, the
...
| | |
|---|---|
| **Diagnose** | Paste a Test Report line or a display message. One card per component with wiring chips, mini-map, callout link, status + note, and a shared-cause check across column, row, connector and EOS mechanics. |
...
| **Status** | OK / Fault / Not tested and a note per component, stored on the device. |
...
- **Docker**. `docker compose up --build` serves on http://localhost:8080. Pass
  `--build-arg BASE_PATH=/TheAddamsFamilyHandbook/` to serve under a sub-path. *Untested locally*
  (no Docker here).
